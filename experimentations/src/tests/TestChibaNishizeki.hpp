...
public:
    ...
    static void testCountChibaNishizekiLowMedHigh(int nbRuns) {
        ...
        for (const string& graphPath : graphsPaths) {
            ...
            cout << "Graph: " << graphPath << " | Function: " << "CountChibaNishizekiLowMedHigh" << " NbRun: "<< nbRuns << endl;
            for (int i = 0; i < nbRuns; ++i) {
                //cout << "Run number: " << i << endl;
                start = chrono::high_resolution_clock::now();
                cpt = g->countTrianglesChibaNishizeki();
            ...
            ResultsEncoder* resultsEncoder = new ResultsEncoder();
            resultsEncoder->encodeResults("../results/ChibaNishizeki/lowmedhigh/CountChibaNishizekiLowMedHigh.txt", graphPath, "CountChibaNishizekiLowMedHigh", times, cpt); 
        }
    }

    static void testFindChibaNishizekiLowMedHigh(int nbRuns) {
        vector<string> graphsPaths = {  "../graphs/Road-Network/roadNet-PA.txt",
                                        "../graphs/p2p-Gnutella/p2p-Gnutella08.txt",
                                        "../graphs/Social-Network/soc-Slashdot0811.txt",
                                        "../graphs/Web-graph/web-NotreDame.txt",
        ...
        for (const string& graphPath : graphsPaths) {
            ...
            cout << "Graph: " << graphPath << " | Function: " << "FindChibaNishizekiLowMedHigh" << " NbRun: "<< nbRuns << endl;
            for (int i = 0; i < nbRuns; ++i) {
                //cout << "Run number: " << i << endl;
                start = chrono::high_resolution_clock::now();
                cpt = g->findTrianglesChibaNishizeki().size();
            ...
            ResultsEncoder* resultsEncoder = new ResultsEncoder();
            resultsEncoder->encodeResults("../results/ChibaNishizeki/lowmedhigh/FindChibaNishizekiLowMedHigh.txt", graphPath, "FindChibaNishizekiLowMedHigh", times, cpt); 
        }
    }
